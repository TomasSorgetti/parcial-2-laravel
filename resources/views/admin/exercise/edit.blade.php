<x-layouts.admin>
    <x-slot:title>Math Spark | Admin Edit Exercise</x-slot:title>
    <x-slot:description>Admin Edit Exercise</x-slot:description>

    <main class="my-32 max-w-3xl mx-auto bg-white p-8 rounded-xl shadow-lg border border-black/5">

        <h1 class="text-3xl font-bold mb-6 text-center">Edit Exercise: {{ $exercise->title }}</h1>

        <form action="{{ route('admin.exercises.update', $exercise->id) }}"
            method="POST"
            ...
            <div>
                ...
                <textarea name="statement" rows="5" class="w-full border p-2 rounded">{{ old('statement', $exercise->statement) }}</textarea>
                @error('statement') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="font-semibold">Solution</label>
                <textarea name="solution" rows="5" class="w-full border p-2 rounded">{{ old('solution', $exercise->solution) }}</textarea>
            ...
            <div>
                ...
                <select name="exam_board" class="w-full border p-2 rounded">
                    <option value="IB" {{ old('exam_board', $exercise->exam_board) == 'IB' ? 'selected' : '' }}>IB</option>
                    <option value="IGCSE" {{ old('exam_board', $exercise->exam_board) == 'IGCSE' ? 'selected' : '' }}>IGCSE</option>
                </select>
            ...
            <div>
                ...
                <select name="category_id" class="w-full border p-2 rounded">
                    ...
                    <option value="{{ $cat->id }}"
                        {{ old('category_id', $exercise->category_id) == $cat->id ? 'selected' : '' }}>
                        {{ $cat->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="level_id" class="w-full border p-2 rounded">
                    ...
                    <option value="{{ $lvl->id }}"
                        {{ old('level_id', $exercise->level_id) == $lvl->id ? 'selected' : '' }}>
            ...
            <div class="flex items-center gap-2">
                <input type="checkbox" name="is_published" value="1"
                    {{ old('is_published', $exercise->is_published) ? 'checked' : '' }}>
